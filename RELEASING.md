# Hướng dẫn publish `adflow-core`/`adflow-admob`/`adflow-compose`

Tài liệu dành cho người maintain repo này - các bước để đưa 1 bản build mới của `adflow-core`/`adflow-admob`/`adflow-compose` lên JitPack.
...
curl -s "https://jitpack.io/com/github/wzlibs/adflow/compose/v0.2.0/compose-v0.2.0.pom"
```
...
- Không cần đổi gì trong `adflow-core/build.gradle.kts`/`adflow-admob/build.gradle.kts`/`adflow-compose/build.gradle.kts` khi ra tag mới - cấu hình `maven-publish` hiện tại đã cung cấp sẵn task `publishToMavenLocal` mà JitPack cần.
- `adflow-compose` là module tùy chọn (chỉ cần cho app dùng Jetpack Compose) - `adflow-core`/`adflow-admob` không phụ thuộc nó, nên consumer XML-only có thể bỏ qua coordinate `compose` khi khai báo dependency.
