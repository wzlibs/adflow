...
val adflowLocalMavenDir = rootDir.resolve("../../path/to/flutter/adflow_flutter/android/local-maven")
...
- Local Maven repo (`android/local-maven/`) phải re-publish thủ công mỗi khi `adflow-core`/`adflow-admob` đổi API - quên publish sẽ khiến plugin build với version cũ một cách âm thầm.
